# Fine-Tuning RoBERTa-base on Multi-Source Emotion Data
This document details the training pipeline script (`train_roberta.py`) designed to fine-tune `roberta-base` for multi-class emotion classification using the merged dataset generated by `data_pipeline.py`.

## Pre-requisites & Setup
Ensure your environment has the required packages:
```bash
pip install transformers[torch] datasets accelerate evaluate scikit-learn pandas
```

The fine-tuning label mapping matches the canonical 7-class schema:
```python
...
## The Fine-Tuning Script (`train_roberta.py`)

Create a script `train_roberta.py` as detailed below to carry out the classification task:
...
## Fine-Tuning Command Instruction
To kick off multi-class classification training, run:
```bash
python train_roberta.py
```
This script validates training progress per epoch, ensuring optimization halts at the peak of **macro F1-score**, maximizing minority class recall.
Once completed, verify that `./models/roberta-artemis/config.json` and `./models/roberta-artemis/pytorch_model.bin` exist.
